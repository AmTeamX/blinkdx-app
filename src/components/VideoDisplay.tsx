...
export default function VideoDisplay({ videoId }: VideoDisplayProps) {
  ...
  const displayIntervalMs = metadata?.fps
    ? Math.max(1000 / metadata.fps, 10)
    : 40;

  // --- LOGIC AND HANDLERS ---
  const fetchImageFramesInBatches = useCallback(
    async (currentVideoId: number, totalVideoFrames: number) => {
        ...
        try {
          ...
          setFrames((prev) => {
            ...
            frames.forEach((base64, index) => {
              if (base64)
                newFrames[i + index] = `data:image/jpeg;base64,${base64}`;
            });
            return newFrames;
          });
        } catch (error) {
  ...
  const chartData = useMemo<ChartData<"line">>(() => {
    const labels = Array.from({ length: totalFrames }, (_, i) =>
      (i + 1).toString(),
    );
    ...
    () => ({
      ...
      onHover: (event, chartElements) => {
        if (event.native?.target) {
          (event.native.target as HTMLElement).style.cursor = chartElements[0]
            ? "pointer"
            : "default";
        }
      },
  ...
  // Calculate current frame line position for overlay
  const calculateLinePosition = () => {
    if (!chartContainerRef.current || totalFrames === 0)
      return { left: 0, visible: false };

    const containerRect = chartContainerRef.current.getBoundingClientRect();
    const chartPadding = 60; // Approximate padding for chart area
    const chartWidth = containerRect.width - chartPadding;

    // Calculate frame position within the chart area
    // Use totalFrames instead of totalFrames - 1 to better align with chart grid
    const framePosition =
      totalFrames > 1
        ? (currentFrameIndex / (totalFrames - 1)) * chartWidth
        : 0;

    // For the last frame, adjust position to align with chart's end
    const adjustedPosition =
      currentFrameIndex === totalFrames - 1
        ? chartWidth - 1 // Slight adjustment to stay within bounds
        : framePosition;

    const left = chartPadding + adjustedPosition;

    return {
      left: left,
      visible: currentFrameIndex >= 0 && currentFrameIndex < totalFrames,
    };
  };
      ...
      <div className="flex flex-col items-center justify-center h-screen bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-500 mt-2">
            Preparing frames and data for Video ID: {videoId}
          </p>
          {totalFrames > 0 && (
            <progress
              className="w-64 mt-4"
              value={frames.filter(Boolean).length}
              max={totalFrames}
            />
          )}
          {error && (
      ...
      <main className="flex-1 grid grid-cols-1 lg:grid-cols-[70%_30%] gap-4 min-h-0">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-white rounded-lg shadow-md p-4 flex flex-col min-h-2/3">
                  ...
                  {currentSvgPaths && (
                    ...
                    >
                      <g
                        ...
                        strokeWidth="2"
                        fill="none"
                        dangerouslySetInnerHTML={{ __html: currentSvgPaths }}
                      />
                    </svg>
                  )}
                </>
            ...
            <div className="pt-4 space-y-3 pl-14">
              <input
                type="range"
                min="0"
              ...
              <div className="flex justify-between items-center text-sm text-gray-700">
                ...
                <span className="font-mono">
                  Time: {(currentFrameIndex / (metadata?.fps || 30)).toFixed(2)}
                  s
                </span>
              </div>
              <div className="flex justify-between items-center text-sm text-gray-700">
            ...
            <div className="relative flex-1 w-full" ref={chartContainerRef}>
              {selectedParams.length > 0 ? (
                ...
                <div className="flex items-center justify-center h-full text-gray-400">
                  Select parameters to view graph.
                </div>
              )}
            </div>
          </div>
